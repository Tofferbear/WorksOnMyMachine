...
class RoleRegistry {
     ...
     */
    static getSortedRoles(room, counts) {
...
class MetricsUtils {
    ...
    static recordStat(creepName, statType, amount) {
        if (!Memory.stats)
            return;
        ...
        Memory.stats[creepName][statType] += amount;
    }
    static setupLeaderboardGlobal() {
        global.leaderboard = function (metric = "energyHarvested") {
            if (!Memory.stats) {
                return "No stats collected yet.";
            }
            const sorted = Object.keys(Memory.stats).sort((a, b) => (Memory.stats[b][metric] || 0) - (Memory.stats[a][metric] || 0));
            console.log(`--- 🦌 ${metric.toUpperCase()} Leaderboard 🦌 ---`);
            sorted.slice(0, 5).forEach((name, index) => {
                const value = Memory.stats[name][metric] || 0;
                if (value > 0) {
    ...
    static exportTelemetry() {
        var _a, _b;
        if (Game.time % 10 !== 0)
            return; // Only update every 10 ticks to save CPU
        ...
        for (const roomName in Game.rooms) {
            ...
            roomsState[roomName] = {
                ...
                remoteRoomsTargeted: ((_a = room.memory.remoteRooms) === null || _a === void 0 ? void 0 : _a.length) || 0,
        ...
        Memory.ml_telemetry = {
            ...
            currentSkills: ((_b = Memory.empire) === null || _b === void 0 ? void 0 : _b.skills) || {},
...
class ScavengeUtils {
     ...
     */
    static tryScavengeEnergy(creep, range = 1) {
...
class StagRole {
    ...
    getPriority(room, counts) {
        const hasNoProducers = counts.stag === 0 && counts.buck === 0 && counts.javan === 0;
        if (hasNoProducers)
            return 1; // Emergency!
        if (room.controller && room.controller.level === 1)
            return 60; // Baseline for RCL 1
        return 999; // Not normally used at higher RCL
    }
    shouldSpawn(room, counts) {
        var _a, _b, _c, _d;
        const hasNoProducers = counts.stag === 0 && counts.buck === 0 && counts.javan === 0;
        if (hasNoProducers)
            return true;
        const limit = Math.max(1, (_d = (_c = (_b = (_a = Memory.empire) === null || _a === void 0 ? void 0 : _a.skills) === null || _b === void 0 ? void 0 : _b[room.name]) === null || _c === void 0 ? void 0 : _c.stagLimit) !== null && _d !== void 0 ? _d : 4); // Hard floor of 1
        if (room.controller && room.controller.level === 1 && counts.stag < limit)
            return true;
        return false;
    }
    generateBody(energy) {
        ...
        let body = [];
        for (let i = 0; i < Math.max(1, numSegments); i++)
            body.push(...segment);
        return body;
    }
    getNewName() {
    ...
    generateName(nameList) {
        if (!Memory.nameCounters)
            Memory.nameCounters = {};
    ...
    run(creep) {
        ...
        if (creep.memory.working) {
            ...
            if (sites.length > 0) {
                ...
                const highestPriority = Math.min(...sites.map(s => { var _a; return (_a = priority[s.structureType]) !== null && _a !== void 0 ? _a : 99; }));
                const prioritySites = sites.filter(s => { var _a; return ((_a = priority[s.structureType]) !== null && _a !== void 0 ? _a : 99) === highestPriority; });
...
class BuckRole {
    ...
    getPriority(room, counts) {
        if (counts.buck < 1 && room.controller && room.controller.level >= 2)
            return 2; // Emergency Bootstrap Priority
    ...
    run(creep) {
        ...
        let targetPos = container ? container.pos : (sourceLink ? sourceLink.pos : source.pos);
...
class StagingUtils {
     ...
     */
    static parkCreep(creep, targetPos) {
            ...
            for (let r = 3; r <= 6; r++) {
                ...
                for (let dx = -r; dx <= r; dx++) {
                    for (let dy = -r; dy <= r; dy++) {
                        ...
                        if (isBlocked || pos.lookFor(LOOK_CREEPS).filter(c => c.id !== creep.id).length > 0)
                            continue;
                        validSpots.push(pos);
                    }
                }
...
class MuleRole {
    ...
    getPriority(room, counts) {
        if (counts.mule < 1 && (counts.buck > 0 || counts.stag > 0))
            return 3; // Emergency Bootstrap Priority
        const sources = room.find(FIND_SOURCES).length;
        if (counts.mule < sources)
            return 90; // Min haulers
        if (counts.mule < sources * 2)
            return 120; // Desired haulers
        return 999;
    }
    shouldSpawn(room, counts) {
        var _a, _b, _c, _d;
        const sources = room.find(FIND_SOURCES).length;
        const limit = (_d = (_c = (_b = (_a = Memory.empire) === null || _a === void 0 ? void 0 : _a.skills) === null || _b === void 0 ? void 0 : _b[room.name]) === null || _c === void 0 ? void 0 : _c.muleLimit) !== null && _d !== void 0 ? _d : (sources * 2);
    ...
    run(creep) {
        ...
        if (creep.memory.working) {
            ...
            let lockedTarget = Game.getObjectById(creep.memory.targetId);
            if (lockedTarget) {
                if (lockedTarget instanceof Resource && lockedTarget.amount === 0)
                    lockedTarget = null;
                else if ('store' in lockedTarget && lockedTarget.store.getUsedCapacity(RESOURCE_ENERGY) === 0)
                    lockedTarget = null;
            }
            if (!lockedTarget) {
                ...
                if (lockedTarget)
                    creep.memory.targetId = lockedTarget.id;
            }
            if (lockedTarget) {
...
class OryxRole {
    ...
    getPriority(room, counts) {
        if (counts.oryx < 1 && (counts.buck > 0 || counts.stag > 0))
            return 4;
        if (counts.oryx < 2)
            return 140;
        return 999;
    ...
    shouldSpawn(room, counts) {
        ...
        if (counts.oryx < 1 && (counts.buck > 0 || counts.stag > 0))
            return true;
        const limit = Math.max(1, (_d = (_c = (_b = (_a = Memory.empire) === null || _a === void 0 ? void 0 : _a.skills) === null || _b === void 0 ? void 0 : _b[room.name]) === null || _c === void 0 ? void 0 : _c.oryxLimit) !== null && _d !== void 0 ? _d : 2); // Hard floor of 1
        if (counts.oryx < limit)
            return true;
        return false;
    }
    generateBody(energy) {
            ...
            if (!sourceTarget) {
                const containers = creep.room.find(FIND_STRUCTURES, {
                    filter: s => {
                        ...
                        if (creep.room.controller && s.pos.getRangeTo(creep.room.controller) <= 3)
                            return false;
                        return true;
                    }
                });
...
class ElkRole {
    ...
    shouldSpawn(room, counts) {
        ...
        if (room.controller.level === 8) {
            ...
            if (stored > 750000)
                desiredElks = 5;
            else if (stored > 500000)
                desiredElks = 4;
            else if (stored > 250000)
                desiredElks = 3;
        ...
        let limit = (_d = (_c = (_b = (_a = Memory.empire) === null || _a === void 0 ? void 0 : _a.skills) === null || _b === void 0 ? void 0 : _b[room.name]) === null || _c === void 0 ? void 0 : _c.elkLimit) !== null && _d !== void 0 ? _d : desiredElks;
    ...
    run(creep) {
        ...
        if (creep.memory.working) {
                ...
                if (result === ERR_NOT_IN_RANGE) {
                    ...
                    if (creep.store.getUsedCapacity(RESOURCE_ENERGY) <= creep.getActiveBodyparts(WORK) * UPGRADE_CONTROLLER_POWER) {
                        ...
                        if (adjacentContainer)
                            creep.withdraw(adjacentContainer, RESOURCE_ENERGY);
                    }
                }
            }
        }
            ...
            let target = null;
            if (creep.room.controller) {
                const controllerLink = creep.room.controller.pos.findInRange(FIND_MY_STRUCTURES, 3, { filter: { structureType: STRUCTURE_LINK } })[0];
                if (controllerLink && controllerLink.store.getUsedCapacity(RESOURCE_ENERGY) > 0) {
                    target = controllerLink;
                }
                if (!target) {
                    const controllerContainer = creep.room.controller.pos.findInRange(FIND_STRUCTURES, 3, { filter: { structureType: STRUCTURE_CONTAINER } })[0];
...
class ConstructUtils {
     ...
     */
    static getPrioritySite(creep) {
        const sites = creep.room.find(FIND_MY_CONSTRUCTION_SITES);
        if (sites.length === 0)
            return null;
        // Group sites by their priority level
        const highestPriorityValue = Math.min(...sites.map(s => { var _a; return (_a = this.PRIORITY[s.structureType]) !== null && _a !== void 0 ? _a : 99; }));
        // Return the closest site within that priority tier
        const prioritySites = sites.filter(s => { var _a; return ((_a = this.PRIORITY[s.structureType]) !== null && _a !== void 0 ? _a : 99) === highestPriorityValue; });
...
class WapitiRole {
    ...
    shouldSpawn(room, counts) {
        ...
        if (!canBuild)
            return false;
        const limit = (_d = (_c = (_b = (_a = Memory.empire) === null || _a === void 0 ? void 0 : _a.skills) === null || _b === void 0 ? void 0 : _b[room.name]) === null || _c === void 0 ? void 0 : _c.wapitiLimit) !== null && _d !== void 0 ? _d : 2;
...
class GazelleRole {
    ...
    run(creep) {
        ...
        if (creep.room.name !== creep.memory.targetRoom) {
            ...
            if (mineral)
                Memory.rooms[creep.room.name].mineral = mineral.id;
            if (creep.room.controller) {
...
class ReindeerRole {
    ...
    shouldSpawn(room, counts) {
        ...
        if (((_a = Memory.empire) === null || _a === void 0 ? void 0 : _a.targetExpansionRoom) && ((_b = Memory.empire) === null || _b === void 0 ? void 0 : _b.expansionRoomClaimed)) {
    ...
    getInitialMemory(room, counts) {
        const mem = { targetRoom: this._pendingTargetRoom };
    ...
    run(creep) {
        ...
        if (!targetRoom) {
            ...
            if ((_a = Memory.empire) === null || _a === void 0 ? void 0 : _a.targetExpansionRoom) {
...
class JavanRole {
    ...
    shouldSpawn(room, counts) {
        var _a, _b, _c, _d;
        if (((_c = (_b = (_a = Memory.empire) === null || _a === void 0 ? void 0 : _a.skills) === null || _b === void 0 ? void 0 : _b.global) === null || _c === void 0 ? void 0 : _c.remoteMiningEnabled) === false)
            return false;
        if (!room.memory.remoteMiningActive)
            return false;
        if (!room.memory.remoteRooms || room.memory.remoteRooms.length === 0)
            return false;
        for (const remoteRoomName of room.memory.remoteRooms) {
            const remoteRoomData = (_d = Memory.rooms) === null || _d === void 0 ? void 0 : _d[remoteRoomName];
            if (!remoteRoomData || !remoteRoomData.sources)
                continue;
            // Skip rooms that have graduated to sovereign bases
            const remoteRoomObj = Game.rooms[remoteRoomName];
            if (remoteRoomObj && remoteRoomObj.find(FIND_MY_SPAWNS).length > 0)
                continue;
    ...
    getInitialMemory(room, counts) {
        const mem = {
            ...
            targetSource: this._pendingTargetSource || undefined
        };
        this._pendingTargetRoom = null;
        this._pendingTargetSource = null;
        return mem;
                ...
                if (creep.transfer(target, RESOURCE_ENERGY) == ERR_NOT_IN_RANGE) {
                    creep.moveTo(target, {
                        ...
                        costCallback: (roomName, costMatrix) => {
                            for (let i = 0; i < 50; i++) {
                                costMatrix.set(0, i, 20);
                                costMatrix.set(49, i, 20);
                                costMatrix.set(i, 0, 20);
                                costMatrix.set(i, 49, 20);
                            }
                            return costMatrix;
                        }
                    });
...
class SchomburgkRole {
    constructor() {
        this.roleName = "schomburgk";
        this._pendingTargetRoom = null;
    }
    getPriority(room, counts) {
        return 125;
    ...
    shouldSpawn(room, counts) {
        ...
        for (const remoteRoomName of room.memory.remoteRooms) {
            const remoteRoomObj = Game.rooms[remoteRoomName];
            if (!remoteRoomObj)
                continue; // Requires vision to detect sites/damage
            // Skip rooms that have graduated to sovereign bases
            if (remoteRoomObj.find(FIND_MY_SPAWNS).length > 0)
                continue;
            const schomburgksTargeting = _.filter(Game.creeps, c => c.memory.role === this.roleName && c.memory.targetRoom === remoteRoomName);
            if (schomburgksTargeting.length > 0)
                continue;
    ...
    run(creep) {
        ...
        if (creep.memory.working) {
            ...
            if (site) {
                ...
                if (damagedStruct) {
                    if (creep.repair(damagedStruct) === ERR_NOT_IN_RANGE)
                        creep.moveTo(damagedStruct);
                }
                else {
                    creep.say("💤 idle");
...
class CaribouRole {
    ...
    shouldSpawn(room, counts) {
        ...
        if (((_d = Memory.empire) === null || _d === void 0 ? void 0 : _d.targetExpansionRoom) && !((_e = Memory.empire) === null || _e === void 0 ? void 0 : _e.expansionRoomClaimed)) {
            const expansionTarget = Memory.empire.targetExpansionRoom;
    ...
    run(creep) {
        ...
        if (creep.room.controller) {
            ...
            if (isExpansion && creep.room.controller.my) {
                creep.say("🏠 home");
                if (Memory.empire)
                    Memory.empire.expansionRoomClaimed = true;
                const homeSpawn = Object.values(Game.spawns).find(s => s.room.name !== targetRoom);
                if (homeSpawn) {
                    if (creep.pos.getRangeTo(homeSpawn) > 1)
                        creep.moveTo(homeSpawn, { reusePath: 20 });
                }
                return;
            }
            let result;
...
class MooseRole {
    ...
    shouldSpawn(room, counts) {
        ...
        if (hostiles.length > 0 || hostileCores.length > 0) {
            ...
            const limit = (_d = (_c = (_b = (_a = Memory.empire) === null || _a === void 0 ? void 0 : _a.skills) === null || _b === void 0 ? void 0 : _b[room.name]) === null || _c === void 0 ? void 0 : _c.militaryLimit) !== null && _d !== void 0 ? _d : 2;
        ...
        if (room.memory.remoteRooms) {
            for (const remoteRoomName of room.memory.remoteRooms) {
                ...
                const limit = (_h = (_g = (_f = (_e = Memory.empire) === null || _e === void 0 ? void 0 : _e.skills) === null || _f === void 0 ? void 0 : _f[remoteRoomName]) === null || _g === void 0 ? void 0 : _g.militaryLimit) !== null && _h !== void 0 ? _h : 2;
...
class SikaRole {
    ...
    shouldSpawn(room, counts) {
        ...
        if (extractor && mineral && mineral.mineralAmount > 0) {
            const limit = (_d = (_c = (_b = (_a = Memory.empire) === null || _a === void 0 ? void 0 : _a.skills) === null || _b === void 0 ? void 0 : _b[room.name]) === null || _c === void 0 ? void 0 : _c.sikaLimit) !== null && _d !== void 0 ? _d : 1;
            ...
            if (mineral) {
                if (mineral.mineralAmount > 0) {
                    ...
                    if (creep.room.storage)
                        creep.moveTo(creep.room.storage, { range: 3 });
                }
            }
        }
    }
    ...
    run(room) {
        ...
        for (const role of sortedRoles) {
            if (role.shouldSpawn(room, counts)) {
                ...
                if (this.spawnCreep(spawn, role, energyToUse, counts))
                    return;
            }
        }
    }
    spawnCreep(spawn, role, energy, counts) {
...
class ConstructionManager {
    ...
    run(room) {
        ...
        if (needsHighPriority ? Game.time % 10 !== 0 : Game.time % 100 !== 0)
            return;
        if (!room.controller || !room.controller.my)
            return;
        ...
        if (spawns.length === 0) {
            ...
            if (spawnSites.length === 0) {
                ...
                const found = this.placeStructureNear(room, new RoomPosition(25, 25, room.name), STRUCTURE_SPAWN, 15, newName);
                if (found) {
        ...
        if (level >= 5) {
            ...
            if (neededLinks > 0) {
                ...
                if (storageLinks.length === 0 && storageSites.length === 0 && neededLinks > 0) {
                    if (this.placeStructureNear(room, center, STRUCTURE_LINK, 2))
                        neededLinks--;
                }
                ...
                if (neededLinks > 0) {
                    ...
                    if (ctrlLinks.length === 0 && ctrlSites.length === 0 && neededLinks > 0) {
                        if (this.placeStructureNear(room, room.controller.pos, STRUCTURE_LINK, 2))
                            neededLinks--;
                    }
                }
                ...
                if (neededLinks > 0 && level >= 6) {
                    ...
                    for (const source of sources) {
                        ...
                        if (sourceLinks.length === 0 && sourceSites.length === 0) {
                            if (this.placeStructureNear(room, source.pos, STRUCTURE_LINK, 1))
                                neededLinks--;
                        }
                    }
                }
    ...
    placeStructureNear(room, pos, structureType, range = 1, name) {
    ...
    run(room) {
        ...
        if (room.storage) {
            ...
            if (stored > 200000)
                ...
                rampartTarget = 300000;
            else if (stored > 50000)
                rampartTarget = 100000;
            else
                rampartTarget = 25000;
        }
...
const BLACKLIST_DURATION_TICKS = 10000;
...
class RemoteMiningManager {
    ...
    run(baseRoom) {
        ...
        if (baseRoom.controller.level < 3)
            return; // Wait until RCL 3 for outposts
        ...
        if (baseRoom.memory.remoteRooms.length < baseRoom.controller.level - 2) {
            if (Memory.rooms) {
                ...
                for (const exit of exits) {
                    const isBase = (_b = (_a = Memory.empire) === null || _a === void 0 ? void 0 : _a.remoteBases) === null || _b === void 0 ? void 0 : _b.includes(exit);
                    if (Memory.rooms[exit] && Memory.rooms[exit].sources && !isBase && !baseRoom.memory.remoteRooms.includes(exit)) {
                        ...
                        if (Memory.rooms[exit].controllerId) {
                            const ctrl = Game.getObjectById(Memory.rooms[exit].controllerId);
                            if (ctrl && (ctrl.owner || ((_c = ctrl.reservation) === null || _c === void 0 ? void 0 : _c.username) !== "Tofferbear"))
                                continue;
                        }
        ...
        for (const remoteRoomName of baseRoom.memory.remoteRooms) {
            ...
            const storageCapacity = (_f = (_e = baseRoom.storage) === null || _e === void 0 ? void 0 : _e.store.getCapacity(RESOURCE_ENERGY)) !== null && _f !== void 0 ? _f : 1;
            const storageUsed = (_h = (_g = baseRoom.storage) === null || _g === void 0 ? void 0 : _g.store.getUsedCapacity(RESOURCE_ENERGY)) !== null && _h !== void 0 ? _h : 0;
     ...
     */
    assessOutpostThreats(baseRoom) {
        var _a, _b, _c;
        if (!baseRoom.memory.remoteRooms)
            return;
        const roomsToAbandon = [];
        for (const remoteRoomName of baseRoom.memory.remoteRooms) {
            const remoteRoom = Game.rooms[remoteRoomName];
            if (!remoteRoom)
                continue; // No vision — can't assess
            if (!Memory.rooms)
                Memory.rooms = {};
            ...
            if (hasThreats) {
                const lastTick = (_a = roomMem.lastHostileTick) !== null && _a !== void 0 ? _a : 0;
                // Only count as a new incident if enough time has passed since the last one
                if (Game.time - lastTick > INCIDENT_COOLDOWN_TICKS) {
                    roomMem.hostileIncidents = ((_b = roomMem.hostileIncidents) !== null && _b !== void 0 ? _b : 0) + 1;
                ...
                if (((_c = roomMem.hostileIncidents) !== null && _c !== void 0 ? _c : 0) >= HOSTILE_INCIDENT_THRESHOLD) {
    ...
    placeInfrastructure(baseRoom) {
        ...
        for (const remoteRoomName of baseRoom.memory.remoteRooms || []) {
            const remoteRoom = Game.rooms[remoteRoomName];
            if (!remoteRoom)
                continue; // Requires active vision
                ...
                if (globalSitesCount < 60) {
                    const ret = PathFinder.search(spawn.pos, { pos: source.pos, range: 1 }, {
                        ...
                        roomCallback: (roomName) => {
                            const room = Game.rooms[roomName];
                            if (!room)
                                return false; // Don't path through blind rooms
...
class HerdExpansionManager {
    run() {
        var _a, _b, _c;
        // Run fast when actively expanding, slow otherwise
        const hasActiveExpansion = ((_a = Memory.empire) === null || _a === void 0 ? void 0 : _a.targetExpansionRoom) != null;
        if (hasActiveExpansion ? Game.time % 10 !== 0 : Game.time % 100 !== 0)
            return;
        // graduation cleanup
        if ((_b = Memory.empire) === null || _b === void 0 ? void 0 : _b.targetExpansionRoom) {
            this.checkInitialGraduation(Memory.empire.targetExpansionRoom);
        }
        // Check for "Mature" graduation (Becoming a full Home Base)
        if ((_c = Memory.empire) === null || _c === void 0 ? void 0 : _c.remoteBases) {
    ...
    checkInitialGraduation(targetRoom) {
        const newRoomObj = Game.rooms[targetRoom];
        if (!newRoomObj || newRoomObj.find(FIND_MY_SPAWNS).length === 0)
            return;
    ...
    checkMatureGraduation(roomName) {
        var _a, _b;
        const room = Game.rooms[roomName];
        if (!room)
            return;
        // Mature milestone: RCL 4 AND has a local energy producer
        const hasProducer = _.some(Game.creeps, c => (c.memory.role === 'buck' || c.memory.role === 'javan') &&
        ...
        if (room.controller && room.controller.level >= 4 && hasProducer) {
            ...
            for (const otherRoomName in Game.rooms) {
                ...
                if ((_b = (_a = otherRoom.memory) === null || _a === void 0 ? void 0 : _a.remoteRooms) === null || _b === void 0 ? void 0 : _b.includes(roomName)) {
    ...
    selectExpansionTarget(baseRoom) {
        if (!Memory.rooms)
            return;
        let bestRoom = null;
        let maxSources = 0;
        ...
        for (const roomName in Memory.rooms) {
            ...
            if (!roomData.sources || roomData.sources.length === 0)
                continue;
            if (!roomData.controllerId)
                continue; // Needs a controller to expand to
...
class HerdSupportManager {
    run() {
        if (!Memory.empire)
            Memory.empire = {};
        if (!Memory.empire.supportTargets)
            Memory.empire.supportTargets = [];
        // Evaluate every 10 ticks to save CPU
        if (Game.time % 10 !== 0)
            return;
        const newSupportTargets = [];
...
// src/managers/MilitaryOperationsManager.ts
class MilitaryOperationsManager {
    run() {
        // High-level military planning (raids, sieges) will go here.
        // Reactive defense and garrisons are now handled autonomously by modular MooseRole.
    }
}
...
class ClientVisualsManager {
    run(room) {
        var _a, _b, _c, _d, _e, _f, _g;
        const isOutlander = (c) => ['sambar', 'javan', 'schomburgk', 'gazelle', 'caribou', 'reindeer'].includes(c.memory.role) ||
            (c.memory.role === 'moose' && !!c.memory.targetRoom);
        const getAgedColor = (baseColor, ttl) => {
            if (ttl === undefined)
                return baseColor;
        ...
        for (const homeName in Game.rooms) {
            ...
            if (((_a = home.controller) === null || _a === void 0 ? void 0 : _a.my) && ((_b = home.memory.remoteRooms) === null || _b === void 0 ? void 0 : _b.includes(roomName))) {
                isOutpost = true;
                break;
            }
        }
        if (room.controller && room.controller.my) {
            const isRemoteBase = (_d = (_c = Memory.empire) === null || _c === void 0 ? void 0 : _c.remoteBases) === null || _d === void 0 ? void 0 : _d.includes(roomName);
            const isExpansion = ((_e = Memory.empire) === null || _e === void 0 ? void 0 : _e.targetExpansionRoom) === roomName;
            if (isExpansion) {
                roomType = "🚀 EXPANSION";
                headerColor = '#ff5555';
        ...
        const drawColumns = (columns, creepsToDraw, startY) => {
            const creepsByRole = lodash.exports.groupBy(creepsToDraw, c => c.memory.role);
        ...
        if (isHomeBase) {
            ...
            const localCreeps = lodash.exports.filter(Game.creeps, c => !isOutlander(c) && (c.room.name === room.name || c.memory.baseRoom === room.name));
            ...
            for (const outpost of remoteRooms) {
                ...
                if (outpost === ((_f = Memory.empire) === null || _f === void 0 ? void 0 : _f.targetExpansionRoom))
                    continue;
                visual.text(`📍 Outpost: ${outpost}`, botLeftX, botLeftY, { align: "left", color: "#00E0FF", font: "bold 0.65 Tahoma" });
                botLeftY += 0.9;
                const outlanders = lodash.exports.filter(Game.creeps, c => c.memory.targetRoom === outpost);
                ...
                const dataFont = { align: "left", color: "#DDDDDD", font: "0.6 monospace" };
            ...
            botLeftY += 1.2;
            const baseGazelles = lodash.exports.filter(Game.creeps, c => c.memory.role === 'gazelle' && c.memory.baseRoom === room.name);
            const baseReindeers = lodash.exports.filter(Game.creeps, c => c.memory.role === 'reindeer' && c.memory.baseRoom === room.name);
            const expansionFont = { align: "left", color: "#DDDDDD", font: "0.6 monospace" };
            ...
            botLeftY += 0.9;
            const targets = (_g = Memory.empire) === null || _g === void 0 ? void 0 : _g.targetExpansionRoom;
            if (targets) {
            ...
            const resourceColors = {
                [RESOURCE_ENERGY]: '#ffe56c',
                [RESOURCE_KEANIUM]: '#da61ff',
            ...
            const drawStackedBar = (title, store, y) => {
...
class HerdManager {
    constructor() {
        ...
        this.herdExpansionManager = new HerdExpansionManager();
        this.herdSupportManager = new HerdSupportManager();
        this.militaryOpsManager = new MilitaryOperationsManager();
        this.visualsManager = new ClientVisualsManager();
